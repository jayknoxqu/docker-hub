...
docker run -d --restart always --name es_honghe1 \
    ...
    -v ./elasticsearch/config/elasticsearch.yml:/usr/share/elasticsearch/config/elasticsearch.yml:ro \
    --ulimit memlock=-1:-1 \
...
docker run -it -d --restart always --name logstash \
    ...
    -v ./logstash/lib/mysql-connector-java-8.0.15.jar:/usr/share/logstash/lib/mysql-connector-java-8.0.15.jar \
    -v ./logstash/config/logstash.yml:/usr/share/logstash/config/logstash.yml:ro \
    -v ./logstash/config/pipelines.yml:/usr/share/logstash/config/pipelines.yml:ro \
